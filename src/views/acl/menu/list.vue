      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button
            ...
            type="text"
            size="mini"
            @click="() => {dialogFormVisible = true,dialogFormValue='添加菜单', menu.pid = scope.row.id}">添加菜单
          </el-button>
          <el-button
          ...
          <el-button
            ...
            type="text"
            size="mini"
            @click="() => {addFun='修改功能',updateFunction(scope.row)}">修改功能
          </el-button>
          <el-button
            v-if="scope.row.level != 4 "
            type="text"
            size="mini"
            @click="() =>{dialogFormValue='修改菜单',getById(scope.row)} ">修改
          </el-button>
          <el-button
            type="text"
            size="mini"
    ...
    <el-dialog :visible.sync="dialogFormVisible" :title="dialogFormValue">
      <el-form ref="menu" :model="menu" :rules="menuValidateRules" label-width="120px">
        <el-form-item label="菜单名称" prop="permissionName">
          <el-input v-model="menu.permissionName"/>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      permission: perForm,
      menuValidateRules: {
        permissionName: [{required: true, trigger: 'blur', message: '菜单名必须输入'}],
      },
      permissionValidateRules: {
        permissionName: [{required: true, trigger: 'blur', message: '功能名称必须输入'}],
        path: [{required: true, trigger: 'blur', message: '访问路径必须输入'}],
        permissionCode: [{required: true, trigger: 'blur', message: '功能权限值必须输入'}]
      }
    }
  },
  created() {
